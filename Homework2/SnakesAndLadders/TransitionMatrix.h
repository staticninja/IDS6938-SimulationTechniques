	...
	for (int j = 0; j < TransitionMatrix.rows(); j++)
		{
			if (j == TransitionMatrix.rows() - 6)
				{
					TransitionMatrix(j, TransitionMatrix.rows() - 1) = prob;
					TransitionMatrix(j, TransitionMatrix.rows() - 2) = prob;
					TransitionMatrix(j, TransitionMatrix.size() - 3) = prob;
					TransitionMatrix(j, TransitionMatrix.size() - 4) = prob;
					TransitionMatrix(j, TransitionMatrix.size() - 5) = prob * 2;
				}/*
